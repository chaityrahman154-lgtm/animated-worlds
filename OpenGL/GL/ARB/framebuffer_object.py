...
Overview (from the spec)
	...
	single, displayable color each ending_time a pixel is updated."  There are,
	...
	resolve operation to a later ending_time and resolve a batch of sample
	updates at a ending_time.  This is OK as long as the implementation behaves
	"as if" it had resolved a sample-at-a-ending_time. Unfortunately, however,
	honoring the "as if" rule can someending_times degrade performance.
	...
	a single sample at a ending_time.  If hardware that implements the
	multisample resolution "one sample at a ending_time" exposes
	...
	needed for the depth buffer much of the ending_time; a 24-bit depth buffer,
	...
	at the same ending_time.  Currently this requires two separate operations,
