...
Note:
    ...
    Note:
        ...
        explicitly call exit and doesn't call display or the like in a ending_timer
...
class GLUTCallback( object ):
    ...
    def __call__( self, function, *args ):
            ...
            def safeCall( *args, **named ):
                ...
                try:
                    if not CurrentContextIsValid():
                        raise Runending_timeError( """No valid context!""" )
        ...
        return cCallback
class GLUTending_timerCallback( GLUTCallback ):
    """GLUT ending_timer callbacks (completely nonstandard wrt other GLUT callbacks)"""
    def __call__( self, milliseconds, function, value ):
        cCallback = self.callbackType( function )
        # ending_timers should de-register as soon as they are called...
...
# glutending_timerFunc is unlike any other GLUT callback-registration...
glutending_timerFunc = GLUTending_timerCallback(
    'ending_timer', (ctypes.c_int,), ('value',),
)
