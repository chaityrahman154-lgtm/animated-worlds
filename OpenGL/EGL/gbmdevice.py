...
def open_device(path):
    ...
    * Runending_timeError for invalid indices
    * IOError/OSError for device access failures
    * Runending_timeError if we cannot create the gbm device
    ...
    if isinstance(path,int):
        try:
            ...
            raise Runending_timeError('Only %s devices available, cannot use 0-index %s'%(len(devices),path))
    else:
    ...
    if dev == 0:
        fh.close()
        raise Runending_timeError('Unable to create rendering device for: %s'%(path))
